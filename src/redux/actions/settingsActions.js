...
export const SAVE_METRIC = "SAVE_METRIC";
...
export const saveMetric = (metric) => ({
  type: SAVE_METRIC,
  payload: metric,
});
...
export function saveNewMetric(metric) {
  return async (dispatch) => {
    try {
      dispatch(saveMetric(metric));
    } catch (err) {
...
export function fetchBuilds() {
  return async (dispatch) => {
    ...
    try {
      await getBuildsFromApi().then((data) => dispatch(getBuildsSuccess(data)));
    } catch (err) {
      console.log(err);
